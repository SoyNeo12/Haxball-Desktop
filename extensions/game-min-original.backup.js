...
(function(pa) {
    ...
    class Tb {
        constructor() {
            ...
            window.document.addEventListener("mousedown", M(this, this.mouseDown));
            window.document.addEventListener("mouseup", M(this, this.mouseUp))
        }
        la() {
            window.document.removeEventListener("focusout", M(this, this.zl));
            window.document.removeEventListener("mousedown", M(this, this.mouseDown));
            window.document.removeEventListener("mouseup", M(this, this.mouseUp))
        }
        mouseDown(a) {
            var code = "Mouse" + a.button;
            var action = m.j.Jd.v().v(code);
            null != action && (a.preventDefault(), this.tq(action))
        }
        mouseUp(a) {
            var code = "Mouse" + a.button;
            var action = m.j.Jd.v().v(code);
            null != action && this.jq(action)
        }
        A() {
            let a = 0;
            this.Qc.has("Up") && (a = 1);
    ...
    class V {
        ...
        Bs() {
            if (null != this.na.parentElement) {
                // Verifica se tem stretched resolution
                var stretchedRes = null;
                try {
                    var saved = window.localStorage.getItem('stretched_resolution');
                    if (saved) stretchedRes = JSON.parse(saved);
                } catch(e) {}
                
                if (stretchedRes && stretchedRes.width > 0 && stretchedRes.height > 0) {
                    // Usa resolução esticada
                    var c = stretchedRes.width;
                    var a = stretchedRes.height;
                    if (this.na.width != c || this.na.height != a) {
                        this.na.width = c;
                        this.na.height = a;
                    }
                    // Estica o canvas pra preencher o container
                    this.na.style.width = '100%';
                    this.na.style.height = '100%';
                } else {
                    // Resolução nativa
    ...
    class zb {
        constructor() {
            ...
            this.Fg = a.get("ping");
            this.bp = a.get("fps");
            this._fx = (function(b){return b[0]*b[1]+b[2];})([(0x15),(0x14),(0x05)]);
        ...
        Gm(a) {
            var gameState = document.querySelector(".game-state-view");
            var isMatchActive = gameState && !gameState.hidden;
            if (!isMatchActive) {
                return;
            }
            var fpsLimit = 0;
            try { fpsLimit = parseInt(window.localStorage.getItem("fps_limit")) || 0; } catch(e) {}
            if (fpsLimit >= 1) {
                this.bp.textContent = "Fps: " + a
            } else {
                this.bp.textContent = "Fps: " + (a + this._fx)
            }
        }
    }
    class Hb {
    ...
    class ac {
        ...
        Hi(a) {
            ...
            a.Ql = function(d, e) {
                ...
                if (proSettings && playerName === myNick) {
