  ...
  function injectSettingsTab(doc) {
    ...
    function renderAvatarSection() {
      ...
      var html =
        ...
        '<div style="margin-bottom:20px;color:var(--theme-text-secondary, #888);font-size:13px;line-height:1.5;">' +
        t('Defina teclas de atalho para trocar de avatar rapidamente durante o jogo.') +
        '</div>';
      ...
      if (bindings.length > 0) {
        ...
        for (var i = 0; i < bindings.length; i++) {
          ...
          html +=
            ...
            i +
            '" style="display:flex;align-items:center;gap:12px;padding:12px;background:var(--theme-bg-secondary, #1a1a1a);border:1px solid var(--theme-border, #232323);border-radius:6px;margin-bottom:8px;">' +
            '<div style="min-width:50px;padding:6px 12px;background:var(--theme-bg-tertiary, #272727);border-radius:4px;text-align:center;color:var(--theme-text-primary, #fff);font-weight:600;font-size:13px;">' +
            keyDisplay +
            '</div>' +
            '<div style="flex:1;color:var(--theme-text-primary, #fff);font-size:14px;">' +
            (b.avatar || '<span style="color:var(--theme-text-muted, #666);">(' + t('vazio') + ')</span>') +
            '</div>' +
            '<button class="edit-avatar-btn" data-index="' +
            i +
            ...
            t('Editar') +
            '</button>' +
            '<button class="remove-avatar-btn" data-index="' +
            i +
            '" style="padding:6px 10px;background:var(--theme-bg-tertiary, #272727);border:none;border-radius:4px;color:#ff4444;cursor:pointer;transition:background 0.15s;"><i class="icon-cancel"></i></button>' +
            '</div>';
        }
        html += '</div>';
      }
      ...
      // Botão de adicionar
      html +=
        '<button id="add-avatar-binding" style="padding:10px 16px;background:var(--theme-bg-tertiary, #272727);border:none;border-radius:6px;color:var(--theme-text-primary, #fff);cursor:pointer;display:flex;align-items:center;gap:8px;font-size:13px;transition:background 0.15s;width:100%;">' +
        '<i class="icon-plus"></i> ' +
        t('Adicionar atalho') +
        '</button></div>';
      ...
      avatarSection.querySelectorAll('.remove-avatar-btn').forEach(function (btn) {
        btn.onmouseenter = function () {
          btn.style.background = 'var(--theme-bg-hover, #333)';
        };
        btn.onmouseleave = function () {
          btn.style.background = 'var(--theme-bg-tertiary, #272727)';
        };
      ...
      if (addBtn) {
        addBtn.onmouseenter = function () {
          addBtn.style.background = 'var(--theme-bg-hover, #333)';
        };
        addBtn.onmouseleave = function () {
          addBtn.style.background = 'var(--theme-bg-tertiary, #272727)';
        };
    ...
    function showEditDialog(doc, index) {
      ...
      dialog.innerHTML =
        ...
        (isNew ? t('Novo Atalho') : t('Editar Atalho')) +
        '</h2>' +
        '<div style="margin-bottom:16px;">' +
        '<label style="display:block;color:var(--theme-text-secondary, #888);font-size:12px;margin-bottom:6px;font-weight:500;">' +
        t('Tecla de Atalho') +
        '</label>' +
        '<button id="key-capture-btn" style="width:100%;padding:12px;background:var(--theme-bg-secondary, #1a1a1a);border:1px solid var(--theme-border-light, #333);border-radius:6px;color:var(--theme-text-primary, #fff);cursor:pointer;text-align:center;font-size:14px;transition:all 0.15s;">' +
        (binding.key
          ? binding.key.replace('Key', '').replace('Digit', '').replace('Numpad', 'Num')
          : t('Clique para definir tecla')) +
        '</button>' +
        '</div>' +
        '<div style="margin-bottom:20px;">' +
        '<label style="display:block;color:var(--theme-text-secondary, #888);font-size:12px;margin-bottom:6px;font-weight:500;">' +
        t('Avatar (emoji ou texto)') +
        '</label>' +
        '<input id="avatar-input" type="text" value="' +
        (binding.avatar || '') +
        '" maxlength="2" style="width:100%;padding:12px;background:var(--theme-bg-secondary, #1a1a1a);border:1px solid var(--theme-border-light, #333);border-radius:6px;color:var(--theme-text-primary, #fff);font-size:18px;text-align:center;box-sizing:border-box;outline:none;transition:border-color 0.15s;" placeholder="🎮">' +
        '</div>' +
        '<div style="display:flex;gap:10px;">' +
        '<button id="cancel-avatar-btn" style="flex:1;padding:12px;background:var(--theme-bg-tertiary, #272727);border:none;border-radius:6px;color:var(--theme-text-primary, #fff);cursor:pointer;font-size:14px;transition:background 0.15s;">' +
        t('Cancelar') +
        '</button>' +
        '<button id="save-avatar-btn" style="flex:1;padding:12px;background:var(--theme-bg-tertiary, #272727);border:none;border-radius:6px;color:var(--theme-text-primary, #fff);cursor:pointer;font-size:14px;font-weight:600;transition:background 0.15s;">' +
        t('Salvar') +
        '</button>' +
        '</div>';
      ...
      // Hover effects
      keyBtn.onmouseenter = function () {
        if (keyBtn.style.borderColor !== 'rgb(245, 158, 11)') keyBtn.style.background = 'var(--theme-bg-hover, #222)';
      };
      keyBtn.onmouseleave = function () {
        if (keyBtn.style.borderColor !== 'rgb(245, 158, 11)')
          keyBtn.style.background = 'var(--theme-bg-secondary, #1a1a1a)';
      };
      avatarInput.onfocus = function () {
        avatarInput.style.borderColor = 'var(--theme-border-light, #444)';
      };
      avatarInput.onblur = function () {
        avatarInput.style.borderColor = 'var(--theme-border-light, #333)';
      };
      cancelBtn.onmouseenter = function () {
        cancelBtn.style.background = 'var(--theme-bg-hover, #333)';
      };
      cancelBtn.onmouseleave = function () {
        cancelBtn.style.background = 'var(--theme-bg-tertiary, #272727)';
      };
        ...
        button.addEventListener(
          ...
          function () {
            ...
            if (hook) {
              ...
              var defaultSection = tabContents.querySelector(
                '.section[data-hook="' + hook.replace('btn', 'sec') + '"]'
              );
              if (defaultSection) {
                // Esconde todas as sections customizadas primeiro
                tabContents
                  .querySelectorAll('.section[data-hook="tokensec"], .section[data-hook="avatarsec"]')
          ...
          true
        ); // Usa capture phase para garantir que seja executado antes
      })(btn);
    });
